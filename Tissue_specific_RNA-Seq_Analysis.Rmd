...
```{r , warning=F, message=F}
tissue <- "stomach" # type: "intestine" or "stomach"
...
# load metadata sheet
metadata <- read.table(paste(dir.path,"metadata_stomach.csv", sep ="/"), sep =";" ) 
...
# set factor levels 
#dds$group <- factor(dds$group) # make sure coldata are factors (not characters or numerics)
...
idx <- rowSums(counts(dds, normalized=TRUE) >=  2) >= 5 #!!! dropped from 5 x 5
dds <- dds[idx,]
